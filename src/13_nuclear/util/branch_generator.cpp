...
out<<"BC MIX  " << branches[b].val1 <<std::endl;
...
//     std::vector<double> p1={0.05,0.05,0.}; double pt1d=0.;
//     std::vector<double> p2={0.05,0.05,1.}; double pt2d=0.;
...
// //      assembly grid
    ...
    for(int ix=0;ix<nrow;ix++){
       ...
       for(int iy=0;iy<ncol;iy++){
          y+=dy;
	  std::vector<double> p_low={x,y,0}; double p_low_v=0.;
	  std::vector<double> p_up={x,y,1};  double p_up_v=0.;
	  branch rod(p_low,p_up,10, p_low_v,p_up_v);
