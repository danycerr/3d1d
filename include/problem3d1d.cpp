...
problem3d1d::solve(void)
{
	...
	//PROJECTION
	//Proietto la velocita' sulla mesh fem della pressione (globale sulla rete)
	gmm::resize(Uproj, dof.Pv()); gmm::clear(Uproj);
	//mass matrix for the pressure
	sparse_matrix_type Mp(dof.Pv(), dof.Pv()); gmm::clear(Mp);
	getfem::asm_mass_matrix(Mp, mimv, mf_Pv);
	
	//termine noto
	vector_type f(dof.Pv()); gmm::clear(f);
	...
	#endif
	gmm::csc_matrix<scalar_type> Mp_aux;
	gmm::clean(Mp, 1E-12);
	gmm::copy(Mp, Mp_aux);
	scalar_type cond;
	gmm::SuperLU_solve(Mp_aux, Uproj, f,cond);
	
	gmm::clear(Mp);
	gmm::clear(f);
	
	//Define the absolute value of the projected velocity
	gmm::resize(Uproj_abs, dof.Pv()); gmm::clear(Uproj_abs);
	for (size_type i=0; i<dof.Pv(); ++i){
		Uproj_abs[i]=fabs(Uproj[i]);
	}
...
// 2 - declaration of variables
	...
	SaveResidual.open("Residuals.txt");	
  ...
  if (PARAM.int_value("VTK_EXPORT"))
  {
	...
	last_dof=nb_branches*mf_Uvi[nb_branches-1].nb_dof();
		...
		for(size_type i=0; i<nb_branches; ++i){
			if(i>0) start += mf_Uvi[i-1].nb_dof();
			length = mf_Uvi[i].nb_dof();
			vector_type vel_i(length); gmm::clear(vel_i);
			gmm::add(gmm::sub_vector(Uv, gmm::sub_interval(start, length)), vel_i);
			
			//creo file txt con le velocita ramo per ramo
			//std::ofstream outUv("Uv"+suff+std::to_string(i)+".txt");
			//outUv << gmm::col_vector(vel_i);
			//outUv.close();
		...
		if(PARAM.int_value("EXPORT_PROJECTED_VEL")){
			//Esporto la velocità globale
			vtk_export exp_Uvproj(descr.OUTPUT+"Uvproj.vtk");
			exp_Uvproj.exporting(mf_Pv);
			exp_Uvproj.write_mesh();
			exp_Uvproj.write_point_data(mf_Pv, Uproj, "Uproj");
		
			//Esporto il valore assoluto della velocità globale
			vtk_export exp_Uvproj_abs(descr.OUTPUT+"Uvproj_abs.vtk");
			exp_Uvproj_abs.exporting(mf_Pv);
			exp_Uvproj_abs.write_mesh();
			exp_Uvproj_abs.write_point_data(mf_Pv, Uproj_abs, "Uproj_abs");
		}
